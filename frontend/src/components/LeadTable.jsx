                  ...
                  {company.top_contacts.map((c, idx) => (
                    <div key={idx} style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr 2fr', gap: '16px', alignItems: 'start', padding: '16px', background: '#ffffff', borderRadius: '6px', border: `1px solid ${theme.border}` }}>
                      ...
                      <select 
                        ...
                        style={{ width: '100%', background: '#ffffff', border: `1px solid ${theme.border}`, color: theme.text, padding: '8px 12px', borderRadius: '4px', fontSize: '13px', outline: 'none', cursor: 'pointer', boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.02)' }}>
                      ...
                      <textarea 
                        ...
                        style={{ width: '100%', background: '#ffffff', border: `1px solid ${theme.border}`, color: theme.text, padding: '10px 12px', borderRadius: '4px', fontSize: '13px', outline: 'none', minHeight: '40px', resize: 'vertical', boxShadow: 'inset 0 1px 2px rgba(0,0,0,0.02)' }} 
                      />
                    </div>
                  ))}
                </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexShrink: 0 }}>
        <div style={{ position: 'relative', width: '360px' }}>
          ...
          <input type="text" placeholder="Search companies or keywords..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{ width: '100%', background: theme.surface, border: `1px solid ${theme.border}`, padding: '10px 12px 10px 36px', borderRadius: '4px', color: theme.text, outline: 'none', fontSize: '14px', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }} />
        </div>
        
        <div style={{ display: 'flex', gap: '12px', alignItems: 'center' }}>
          {!isHistoryView && <StatusPill status={jobStatus} error={error} />}
          <button onClick={handleExportCSV} style={{ display: 'flex', alignItems: 'center', gap: '8px', background: theme.surface, border: `1px solid ${theme.border}`, color: theme.text, padding: '10px 16px', borderRadius: '4px', cursor: 'pointer', fontSize: '13px', fontWeight: '600', boxShadow: '0 1px 2px rgba(0,0,0,0.02)' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', paddingRight: '12px' }}>
        {/* GROUP BY SEARCH PARAMS IF IN HISTORY VIEW */}
        {isHistoryView ? (
          Object.entries(
